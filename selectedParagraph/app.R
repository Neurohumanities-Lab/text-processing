...
server <- function(input, output, session) {
  ...
  output$secondFilter <- renderUI({
    checkboxGroupInput("filter", "Filter:",
                       choiceValues = 1:input$number,
                       choiceNames = 1:input$number,
                       selected=NULL)
  })
  ...
  tops <-reactive({
    if (is.null(input$filter)){
      ...
      corpus %>% 
        filter(sentence%in% names(tail(sort(similarities()),input$number))) %>% 
        slice(-c(as.integer(input$filter)))
    }
  }
  )
  ...
  observeEvent(input$add, {
    ...
    if(counter$countervalue>nrow(tops())) {
